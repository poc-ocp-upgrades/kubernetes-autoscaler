...
func TestMarksTestBeginning(t *testing.T) {
	...
	var testCases = []struct {
		name		string
		testLine	string
	}{{name: "default", testLine: "=== BEGIN TEST CASE ==="}, {name: "failed print before", testLine: "some other text=== BEGIN TEST CASE ==="}, {name: "failed print after", testLine: "=== BEGIN TEST CASE ===some other text after"}}
...
func TestExtractResult(t *testing.T) {
	...
	}{{name: "success", testLine: `SUCCESS after 0.046s: hack/../test/cmd/basicresources.sh:35: executing 'oc delete pods hello-openshift' expecting success`, expectedResult: api.TestResultPass}, {name: "failure", testLine: `FAILURE after 30.239s: hack/../test/cmd/builds.sh:68: executing 'oc new-build -D "FROM centos:7" -o json | python -m json.tool' expecting success: the command returned the wrong error code`, expectedResult: api.TestResultFail}, {name: "try until failure", testLine: `SUCCESS after 0.044s: hack/../test/cmd/basicresources.sh:41: executing 'oc label pod/hello-openshift acustom=label' expecting success; re-trying every 0.2s until completion or 60.000s`, expectedResult: api.TestResultPass}}
	parser := newTestDataParser()
	for _, testCase := range testCases {
...
func TestMarksTestCompletion(t *testing.T) {
	...
	var testCases = []struct {
		name		string
		testLine	string
	}{{name: "default", testLine: "=== END TEST CASE ==="}, {name: "failed print before", testLine: "some other text=== END TEST CASE ==="}, {name: "failed print after", testLine: "=== END TEST CASE ===some other text after"}}
	parser := newTestDataParser()
	for _, testCase := range testCases {
...
func TestMarksSuiteBeginning(t *testing.T) {
	...
	var testCases = []struct {
		name		string
		testLine	string
	}{{name: "basic", testLine: "=== BEGIN TEST SUITE package/name ==="}, {name: "numeric", testLine: "=== BEGIN TEST SUITE 1234 ==="}, {name: "url", testLine: "=== BEGIN TEST SUITE github.com/maintainer/repository/package/file ==="}, {name: "failed print", testLine: `some other textok=== BEGIN TEST SUITE package/name ===`}}
...
func TestExtractSuiteName(t *testing.T) {
	...
	var testCases = []struct {
		...
		testLine	string
		expectedName	string
	}{{name: "basic", testLine: "=== BEGIN TEST SUITE package/name ===", expectedName: "package/name"}, {name: "numeric", testLine: "=== BEGIN TEST SUITE 1234 ===", expectedName: "1234"}, {name: "url", testLine: "=== BEGIN TEST SUITE github.com/maintainer/repository/package/file ===", expectedName: "github.com/maintainer/repository/package/file"}, {name: "failed print", testLine: `some other text=== BEGIN TEST SUITE package/name ===`, expectedName: "package/name"}}
	parser := newTestSuiteDataParser()
	for _, testCase := range testCases {
		actual, contained := parser.ExtractName(testCase.testLine)
		if !contained {
...
func TestMarksSuiteCompletion(t *testing.T) {
	...
	var testCases = []struct {
		name		string
		testLine	string
	}{{name: "basic", testLine: "=== END TEST SUITE ==="}, {name: "failed print", testLine: `some other text=== END TEST SUITE ===`}}
	parser := newTestSuiteDataParser()
	for _, testCase := range testCases {
		if !parser.MarksCompletion(testCase.testLine) {
