...
type resourceRecommendation struct{ target, lower, upper int64 }
...
func getResourceRecommendation(containerRecommendation *vpa_types.RecommendedContainerResources, r apiv1.ResourceName) resourceRecommendation {
 ...
 return resourceRecommendation{target: getOrZero(containerRecommendation.Target), lower: getOrZero(containerRecommendation.LowerBound), upper: getOrZero(containerRecommendation.UpperBound)}
}
...
func (o *observer) OnUpdate(oldObj, newObj interface{}) {
 ...
 result := recommendationChange{oldMissing: !oldFound, newMissing: !newFound, diff: newRecommendation.sub(&oldRecommendation)}
 go func() {
  o.channel <- result
 }()
}
func getVpaObserver(vpaClientSet *vpa_clientset.Clientset) *observer {
 ...
 _, controller := cache.NewIndexerInformer(vpaListWatch, &vpa_types.VerticalPodAutoscaler{}, 1*time.Hour, &vpaObserver, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
...
var _ = RecommenderE2eDescribe("Checkpoints", func() {
 ...
 ginkgo.It("with missing VPA objects are garbage collected", func() {
  ...
  checkpoint := vpa_types.VerticalPodAutoscalerCheckpoint{ObjectMeta: metav1.ObjectMeta{Name: "test", Namespace: ns}, Spec: vpa_types.VerticalPodAutoscalerCheckpointSpec{VPAObjectName: "some-vpa"}}
...
var _ = RecommenderE2eDescribe("VPA CRD object", func() {
 ...
 ginkgo.BeforeEach(func() {
  ...
  config, err := framework.LoadConfig()
  gomega.Expect(err).NotTo(gomega.HaveOccurred())
  vpaCRD = NewVPA(f, "hamster-vpa", &metav1.LabelSelector{MatchLabels: map[string]string{"app": "hamster"}})
  vpaClientSet = vpa_clientset.NewForConfigOrDie(config)
