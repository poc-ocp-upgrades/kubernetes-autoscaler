...
func makePodMetrics(snaps []*ContainerMetricsSnapshot) metricsapi.PodMetrics {
	...
	defer _logClusterCodePath()
	firstSnap := snaps[0]
	podMetrics := metricsapi.PodMetrics{ObjectMeta: metav1.ObjectMeta{Namespace: firstSnap.ID.Namespace, Name: firstSnap.ID.PodName}, Timestamp: metav1.Time{Time: firstSnap.SnapshotTime}, Window: metav1.Duration{Duration: firstSnap.SnapshotWindow}, Containers: make([]metricsapi.ContainerMetrics, len(snaps))}
	for i, snap := range snaps {
		resourceList := calculateResourceList(snap.Usage)
		podMetrics.Containers[i] = metricsapi.ContainerMetrics{Name: snap.ID.ContainerName, Usage: resourceList}
	}
	return podMetrics
}
func calculateResourceList(usage model.Resources) k8sapiv1.ResourceList {
	...
	resourceMap := map[k8sapiv1.ResourceName]resource.Quantity{k8sapiv1.ResourceCPU: resource.MustParse(cpuQuantityString), k8sapiv1.ResourceMemory: resource.MustParse(memoryQuantityString)}
