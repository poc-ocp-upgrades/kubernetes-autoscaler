...
func TestRunOnce(t *testing.T) {
	...
	vpaObj := test.VerticalPodAutoscaler().WithContainer(containerName).WithTarget("2", "200M").WithMinAllowed("1", "100M").WithMaxAllowed("3", "1G").WithSelector(selector).Get()
	...
	updater := &updater{vpaLister: vpaLister, podLister: podLister, evictionFactory: factory, recommendationProcessor: &test.FakeRecommendationProcessor{}}
	updater.RunOnce()
