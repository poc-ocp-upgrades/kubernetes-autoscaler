...
func TestOKWithScaleUp(t *testing.T) {
	...
	clusterstate := NewClusterStateRegistry(provider, ClusterStateRegistryConfig{MaxTotalUnreadyPercentage: 10, OkTotalUnreadyCount: 1}, fakeLogRecorder, newBackoff())
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 4, Time: now, ExpectedAddTime: now.Add(time.Minute)})
	err := clusterstate.UpdateNodes([]*apiv1.Node{ng1_1, ng2_1}, nil, now)
	assert.NoError(t, err)
	assert.True(t, clusterstate.IsClusterHealthy())
	status := clusterstate.GetStatus(now)
	assert.Equal(t, api.ClusterAutoscalerInProgress, api.GetConditionByType(api.ClusterAutoscalerScaleUp, status.ClusterwideConditions).Status)
	assert.Equal(t, 2, len(status.NodeGroupStatuses))
	ng1Checked := false
	ng2Checked := true
	for _, nodeStatus := range status.NodeGroupStatuses {
		if nodeStatus.ProviderID == "ng1" {
			assert.Equal(t, api.ClusterAutoscalerInProgress, api.GetConditionByType(api.ClusterAutoscalerScaleUp, nodeStatus.Conditions).Status)
			ng1Checked = true
		}
		if nodeStatus.ProviderID == "ng2" {
			assert.Equal(t, api.ClusterAutoscalerNoActivity, api.GetConditionByType(api.ClusterAutoscalerScaleUp, nodeStatus.Conditions).Status)
			ng2Checked = true
		}
	}
...
func TestEmptyOK(t *testing.T) {
	...
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 3, Time: now.Add(-3 * time.Second), ExpectedAddTime: now.Add(1 * time.Minute)})
...
func TestOKOneUnreadyNode(t *testing.T) {
	...
	assert.True(t, clusterstate.IsNodeGroupHealthy("ng1"))
	status := clusterstate.GetStatus(now)
	assert.Equal(t, api.ClusterAutoscalerHealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, status.ClusterwideConditions).Status)
	assert.Equal(t, api.ClusterAutoscalerNoActivity, api.GetConditionByType(api.ClusterAutoscalerScaleUp, status.ClusterwideConditions).Status)
	assert.Equal(t, 2, len(status.NodeGroupStatuses))
	ng1Checked := false
	for _, nodeStatus := range status.NodeGroupStatuses {
		if nodeStatus.ProviderID == "ng1" {
			assert.Equal(t, api.ClusterAutoscalerHealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, nodeStatus.Conditions).Status)
			ng1Checked = true
		}
	}
	assert.True(t, ng1Checked)
...
func TestOKOneUnreadyNodeWithScaleDownCandidate(t *testing.T) {
	...
	assert.Equal(t, api.ClusterAutoscalerHealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, status.ClusterwideConditions).Status)
	assert.Equal(t, api.ClusterAutoscalerNoActivity, api.GetConditionByType(api.ClusterAutoscalerScaleUp, status.ClusterwideConditions).Status)
	assert.Equal(t, api.ClusterAutoscalerCandidatesPresent, api.GetConditionByType(api.ClusterAutoscalerScaleDown, status.ClusterwideConditions).Status)
	assert.Equal(t, 2, len(status.NodeGroupStatuses))
	ng1Checked := false
	ng2Checked := false
	for _, nodeStatus := range status.NodeGroupStatuses {
		if nodeStatus.ProviderID == "ng1" {
			assert.Equal(t, api.ClusterAutoscalerHealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, nodeStatus.Conditions).Status)
			assert.Equal(t, api.ClusterAutoscalerCandidatesPresent, api.GetConditionByType(api.ClusterAutoscalerScaleDown, nodeStatus.Conditions).Status)
			ng1Checked = true
		}
		if nodeStatus.ProviderID == "ng2" {
			assert.Equal(t, api.ClusterAutoscalerHealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, nodeStatus.Conditions).Status)
			assert.Equal(t, api.ClusterAutoscalerNoCandidates, api.GetConditionByType(api.ClusterAutoscalerScaleDown, nodeStatus.Conditions).Status)
			ng2Checked = true
		}
	}
	assert.True(t, ng1Checked)
...
func TestMissingNodes(t *testing.T) {
	...
	for _, nodeStatus := range status.NodeGroupStatuses {
		if nodeStatus.ProviderID == "ng1" {
			assert.Equal(t, api.ClusterAutoscalerUnhealthy, api.GetConditionByType(api.ClusterAutoscalerHealth, nodeStatus.Conditions).Status)
			ng1Checked = true
		}
	}
	assert.True(t, ng1Checked)
...
func TestExpiredScaleUp(t *testing.T) {
	...
	fakeClient := &fake.Clientset{}
	fakeLogRecorder, _ := utils.NewStatusMapRecorder(fakeClient, "kube-system", kube_record.NewFakeRecorder(5), false)
	clusterstate := NewClusterStateRegistry(provider, ClusterStateRegistryConfig{MaxTotalUnreadyPercentage: 10, OkTotalUnreadyCount: 1, MaxNodeProvisionTime: 2 * time.Minute}, fakeLogRecorder, newBackoff())
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 4, Time: now.Add(-3 * time.Minute), ExpectedAddTime: now.Add(-1 * time.Minute)})
...
func TestRegisterScaleDown(t *testing.T) {
	...
	clusterstate := NewClusterStateRegistry(provider, ClusterStateRegistryConfig{MaxTotalUnreadyPercentage: 10, OkTotalUnreadyCount: 1}, fakeLogRecorder, newBackoff())
	now := time.Now()
	clusterstate.RegisterScaleDown(&ScaleDownRequest{NodeGroup: provider.GetNodeGroup("ng1"), NodeName: "ng1-1", ExpectedDeleteTime: now.Add(time.Minute), Time: now})
...
func TestUnregisteredNodes(t *testing.T) {
	...
	fakeClient := &fake.Clientset{}
	fakeLogRecorder, _ := utils.NewStatusMapRecorder(fakeClient, "kube-system", kube_record.NewFakeRecorder(5), false)
	clusterstate := NewClusterStateRegistry(provider, ClusterStateRegistryConfig{MaxTotalUnreadyPercentage: 10, OkTotalUnreadyCount: 1, MaxNodeProvisionTime: 10 * time.Second}, fakeLogRecorder, newBackoff())
...
func TestUpdateLastTransitionTimes(t *testing.T) {
	...
	oldStatus := &api.ClusterAutoscalerStatus{ClusterwideConditions: make([]api.ClusterAutoscalerCondition, 0), NodeGroupStatuses: make([]api.NodeGroupStatus, 0)}
	oldStatus.ClusterwideConditions = append(oldStatus.ClusterwideConditions, api.ClusterAutoscalerCondition{Type: api.ClusterAutoscalerHealth, Status: api.ClusterAutoscalerHealthy, LastProbeTime: now, LastTransitionTime: now})
	oldStatus.ClusterwideConditions = append(oldStatus.ClusterwideConditions, api.ClusterAutoscalerCondition{Type: api.ClusterAutoscalerScaleUp, Status: api.ClusterAutoscalerInProgress, LastProbeTime: now, LastTransitionTime: now})
	oldStatus.NodeGroupStatuses = append(oldStatus.NodeGroupStatuses, api.NodeGroupStatus{ProviderID: "ng1", Conditions: oldStatus.ClusterwideConditions})
	newStatus := &api.ClusterAutoscalerStatus{ClusterwideConditions: make([]api.ClusterAutoscalerCondition, 0), NodeGroupStatuses: make([]api.NodeGroupStatus, 0)}
	newStatus.ClusterwideConditions = append(newStatus.ClusterwideConditions, api.ClusterAutoscalerCondition{Type: api.ClusterAutoscalerHealth, Status: api.ClusterAutoscalerHealthy, LastProbeTime: later})
	newStatus.ClusterwideConditions = append(newStatus.ClusterwideConditions, api.ClusterAutoscalerCondition{Type: api.ClusterAutoscalerScaleUp, Status: api.ClusterAutoscalerNotNeeded, LastProbeTime: later})
	newStatus.ClusterwideConditions = append(newStatus.ClusterwideConditions, api.ClusterAutoscalerCondition{Type: api.ClusterAutoscalerScaleDown, Status: api.ClusterAutoscalerNoCandidates, LastProbeTime: later})
	newStatus.NodeGroupStatuses = append(newStatus.NodeGroupStatuses, api.NodeGroupStatus{ProviderID: "ng2", Conditions: newStatus.ClusterwideConditions})
	newStatus.NodeGroupStatuses = append(newStatus.NodeGroupStatuses, api.NodeGroupStatus{ProviderID: "ng1", Conditions: newStatus.ClusterwideConditions})
	...
	expectedNgTimestamps["ng1"] = map[api.ClusterAutoscalerConditionType]metav1.Time{api.ClusterAutoscalerHealth: now, api.ClusterAutoscalerScaleUp: later, api.ClusterAutoscalerScaleDown: later}
	expectedNgTimestamps["ng2"] = map[api.ClusterAutoscalerConditionType]metav1.Time{api.ClusterAutoscalerHealth: later, api.ClusterAutoscalerScaleUp: later, api.ClusterAutoscalerScaleDown: later}
...
func TestScaleUpBackoff(t *testing.T) {
	...
	fakeLogRecorder, _ := utils.NewStatusMapRecorder(fakeClient, "kube-system", kube_record.NewFakeRecorder(5), false)
	clusterstate := NewClusterStateRegistry(provider, ClusterStateRegistryConfig{MaxTotalUnreadyPercentage: 10, OkTotalUnreadyCount: 1}, fakeLogRecorder, newBackoff())
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 1, Time: now.Add(-3 * time.Minute), ExpectedAddTime: now.Add(-1 * time.Minute)})
	...
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 1, Time: now.Add(-2 * time.Minute), ExpectedAddTime: now.Add(-1 * time.Second)})
	err = clusterstate.UpdateNodes([]*apiv1.Node{ng1_1, ng1_2, ng1_3}, nil, now)
	assert.NoError(t, err)
	assert.True(t, clusterstate.IsClusterHealthy())
	assert.True(t, clusterstate.IsNodeGroupHealthy("ng1"))
	...
	now = now.Add(InitialNodeGroupBackoffDuration).Add(time.Second)
	assert.False(t, clusterstate.IsNodeGroupSafeToScaleUp(ng1, now))
	clusterstate.RegisterScaleUp(&ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng1"), Increase: 1, Time: now, ExpectedAddTime: now.Add(time.Second)})
...
func TestIsNodeStillStarting(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedResult	bool
	}{{"unready", apiv1.NodeReady, apiv1.ConditionFalse, true}, {"readiness unknown", apiv1.NodeReady, apiv1.ConditionUnknown, true}, {"out of disk", apiv1.NodeOutOfDisk, apiv1.ConditionTrue, true}, {"network unavailable", apiv1.NodeNetworkUnavailable, apiv1.ConditionTrue, true}, {"started", apiv1.NodeReady, apiv1.ConditionTrue, false}}
	now := time.Now()
