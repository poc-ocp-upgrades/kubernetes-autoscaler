...
type testReason struct{ message string }
...
func TestEventingScaleUpStatusProcessor(t *testing.T) {
	...
	reasons := map[string]Reasons{"group 1": notSchedulableReason, "group 2": notSchedulableReason, "group 3": alsoNotSchedulableReason}
	testCases := []struct {
		caseName		string
		state			*ScaleUpStatus
		expectedTriggered	int
		expectedNoTriggered	int
	}{{caseName: "No scale up", state: &ScaleUpStatus{ScaleUpInfos: []nodegroupset.ScaleUpInfo{}, PodsRemainUnschedulable: []NoScaleUpInfo{{p1, reasons, reasons}, {p2, reasons, reasons}}}, expectedNoTriggered: 2}, {caseName: "Scale up", state: &ScaleUpStatus{ScaleUpInfos: []nodegroupset.ScaleUpInfo{{}}, PodsTriggeredScaleUp: []*apiv1.Pod{p3}, PodsRemainUnschedulable: []NoScaleUpInfo{{p1, reasons, reasons}, {p2, reasons, reasons}}}, expectedTriggered: 1, expectedNoTriggered: 2}}
	for _, tc := range testCases {
		fakeRecorder := kube_record.NewFakeRecorder(5)
		context := &context.AutoscalingContext{AutoscalingKubeClients: context.AutoscalingKubeClients{Recorder: fakeRecorder}}
...
func TestReasonsMessage(t *testing.T) {
	...
	rejected := map[string]Reasons{"group 1": notSchedulableReason, "group 2": notSchedulableReason, "group 3": alsoNotSchedulableReason}
	skipped := map[string]Reasons{"group 4": maxLimitReached, "group 5": notReady, "group 6": maxLimitReached}
	expected := []string{"2 not schedulable", "1 also not schedulable", "2 max limit reached", "1 not ready"}
