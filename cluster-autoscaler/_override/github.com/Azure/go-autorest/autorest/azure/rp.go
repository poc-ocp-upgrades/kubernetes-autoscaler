 ...
 return func(s autorest.Sender) autorest.Sender {
  return autorest.SenderFunc(func(r *http.Request) (resp *http.Response, err error) {
   ...
   for currentAttempt := 0; currentAttempt < client.RetryAttempts; currentAttempt++ {
    ...
    resp, err = autorest.SendWithSender(s, rr.Request(), autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
    if err != nil {
     return resp, err
    }
...
func register(client autorest.Client, originalReq *http.Request, re RequestError) error {
 ...
 newURL := url.URL{Scheme: originalReq.URL.Scheme, Host: originalReq.URL.Host}
 pathParameters := map[string]interface{}{"resourceProviderNamespace": autorest.Encode("path", providerName), "subscriptionId": autorest.Encode("path", subID)}
 const APIVersion = "2016-09-01"
 queryParameters := map[string]interface{}{"api-version": APIVersion}
 preparer := autorest.CreatePreparer(autorest.AsPost(), autorest.WithBaseURL(newURL.String()), autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/{resourceProviderNamespace}/register", pathParameters), autorest.WithQueryParameters(queryParameters))
 req, err := preparer.Prepare(&http.Request{})
 if err != nil {
  return err
 }
 req = req.WithContext(originalReq.Context())
 resp, err := autorest.SendWithSender(client, req, autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
 if err != nil {
  return err
 }
 ...
 err = autorest.Respond(resp, WithErrorUnlessStatusCode(http.StatusOK), autorest.ByUnmarshallingJSON(&provider), autorest.ByClosing())
 if err != nil {
  return err
 }
 ...
 for err == nil && (client.PollingDuration == 0 || (client.PollingDuration != 0 && time.Since(registrationStartTime) < client.PollingDuration)) {
  preparer := autorest.CreatePreparer(autorest.AsGet(), autorest.WithBaseURL(newURL.String()), autorest.WithPathParameters("/subscriptions/{subscriptionId}/providers/{resourceProviderNamespace}", pathParameters), autorest.WithQueryParameters(queryParameters))
  req, err = preparer.Prepare(&http.Request{})
  if err != nil {
   return err
  }
