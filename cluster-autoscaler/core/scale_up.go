...
func ScaleUp(context *context.AutoscalingContext, processors *ca_processors.AutoscalingProcessors, clusterStateRegistry *clusterstate.ClusterStateRegistry, unschedulablePods []*apiv1.Pod, nodes []*apiv1.Node, daemonSets []*extensionsv1.DaemonSet, nodeInfos map[string]*schedulercache.NodeInfo) (*status.ScaleUpStatus, errors.AutoscalerError) {
	...
	if errCP != nil {
		return &status.ScaleUpStatus{Result: status.ScaleUpError}, errors.ToAutoscalerError(errors.CloudProviderError, errCP)
	}
	...
	for nodeGroup, numberOfNodes := range clusterStateRegistry.GetUpcomingNodes() {
		...
		if !found {
			return &status.ScaleUpStatus{Result: status.ScaleUpError}, errors.NewAutoscalerError(errors.InternalError, "failed to find template node for node group %s", nodeGroup)
		}
	...
	for _, nodeGroup := range nodeGroups {
		...
		option := expander.Option{NodeGroup: nodeGroup, Pods: make([]*apiv1.Pod, 0)}
	...
	if bestOption != nil && bestOption.NodeCount > 0 {
		...
		if context.MaxNodesTotal > 0 && len(nodes)+newNodes+len(upcomingNodes) > context.MaxNodesTotal {
			...
			if newNodes < 1 {
				return &status.ScaleUpStatus{Result: status.ScaleUpError}, errors.NewAutoscalerError(errors.TransientError, "max node total count already reached")
			}
		}
		...
		if !found {
			...
			return &status.ScaleUpStatus{Result: status.ScaleUpError}, errors.NewAutoscalerError(errors.CloudProviderError, "No node info for best expansion option!")
		}
		...
		clusterStateRegistry.Recalculate()
		return &status.ScaleUpStatus{Result: status.ScaleUpSuccessful, ScaleUpInfos: scaleUpInfos, PodsRemainUnschedulable: getRemainingPods(podsRemainUnschedulable, skippedNodeGroups), PodsTriggeredScaleUp: bestOption.Pods, PodsAwaitEvaluation: getPodsAwaitingEvaluation(unschedulablePods, podsRemainUnschedulable, bestOption.Pods)}, nil
	}
	return &status.ScaleUpStatus{Result: status.ScaleUpNoOptionsAvailable, PodsRemainUnschedulable: getRemainingPods(podsRemainUnschedulable, skippedNodeGroups)}, nil
}
...
func getPodsPredicatePassingCheckFunctions(context *context.AutoscalingContext, unschedulablePods []*apiv1.Pod, nodeInfos map[string]*schedulercache.NodeInfo) podsPredicatePassingCheckFunctions {
	...
	return podsPredicatePassingCheckFunctions{getPodsPassingPredicates: func(nodeGroupId string) ([]*apiv1.Pod, error) {
...
func executeScaleUp(context *context.AutoscalingContext, clusterStateRegistry *clusterstate.ClusterStateRegistry, info nodegroupset.ScaleUpInfo, gpuType string) errors.AutoscalerError {
	...
	if err := info.Group.IncreaseSize(increase); err != nil {
		...
		return errors.NewAutoscalerError(errors.CloudProviderError, "failed to increase node group size: %v", err)
	}
	clusterStateRegistry.RegisterScaleUp(&clusterstate.ScaleUpRequest{NodeGroup: info.Group, Increase: increase, Time: time.Now(), ExpectedAddTime: time.Now().Add(context.MaxNodeProvisionTime)})
	metrics.RegisterScaleUp(increase, gpuType)
	context.LogRecorder.Eventf(apiv1.EventTypeNormal, "ScaledUpGroup", "Scale-up: group %s size set to %d", info.Group.Id(), info.NewSize)
	return nil
}
func applyScaleUpResourcesLimits(newNodes int, scaleUpResourcesLeft scaleUpResourcesLimits, nodeInfo *schedulercache.NodeInfo, nodeGroup cloudprovider.NodeGroup, resourceLimiter *cloudprovider.ResourceLimiter) (int, errors.AutoscalerError) {
	...
	for resource, resourceDelta := range delta {
		...
		if limit == scaleUpLimitUnknown {
			return 0, errors.NewAutoscalerError(errors.InternalError, fmt.Sprintf("limit unknown for resource %s", resource))
		}
		...
		if newNodes < 1 {
			return 0, errors.NewAutoscalerError(errors.InternalError, fmt.Sprintf("cannot create any node; max limit for resource %s reached", resource))
		}
	}
	return newNodes, nil
}
