...
func TestFindUnneededNodes(t *testing.T) {
	...
	n5.Annotations = map[string]string{ScaleDownDisabledKey: "true"}
	...
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.35, ExpendablePodsPriorityCutoff: 10, UnremovableNodeRecheckTimeout: 5 * time.Minute}
	...
	sd.UpdateUnneededNodes([]*apiv1.Node{n1, n2, n3, n4, n5, n7, n8, n9}, []*apiv1.Node{n1, n2, n3, n4, n5, n6, n7, n8, n9}, []*apiv1.Pod{p1, p2, p3, p4, p5, p6}, time.Now(), nil)
...
func TestPodsWithPrioritiesFindUnneededNodes(t *testing.T) {
	...
	provider.AddNode("ng1", n3)
	provider.AddNode("ng1", n4)
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.35, ExpendablePodsPriorityCutoff: 10}
	context := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	sd := NewScaleDown(&context, clusterStateRegistry)
	sd.UpdateUnneededNodes([]*apiv1.Node{n1, n2, n3, n4}, []*apiv1.Node{n1, n2, n3, n4}, []*apiv1.Pod{p1, p2, p3, p4, p5, p6, p7}, time.Now(), nil)
...
func TestFindUnneededMaxCandidates(t *testing.T) {
	...
	numCandidates := 30
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.35, ScaleDownNonEmptyCandidatesCount: numCandidates, ScaleDownCandidatesPoolRatio: 1, ScaleDownCandidatesPoolMinCount: 1000}
	context := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	sd := NewScaleDown(&context, clusterStateRegistry)
...
func TestFindUnneededEmptyNodes(t *testing.T) {
	...
	numCandidates := 30
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.35, ScaleDownNonEmptyCandidatesCount: numCandidates, ScaleDownCandidatesPoolRatio: 1.0, ScaleDownCandidatesPoolMinCount: 1000}
	context := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	sd := NewScaleDown(&context, clusterStateRegistry)
	sd.UpdateUnneededNodes(nodes, nodes, pods, time.Now(), nil)
...
func TestFindUnneededNodePool(t *testing.T) {
	...
	numCandidates := 30
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.35, ScaleDownNonEmptyCandidatesCount: numCandidates, ScaleDownCandidatesPoolRatio: 0.1, ScaleDownCandidatesPoolMinCount: 10}
	context := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	sd := NewScaleDown(&context, clusterStateRegistry)
	sd.UpdateUnneededNodes(nodes, nodes, pods, time.Now(), nil)
...
func TestDeleteNode(t *testing.T) {
	...
	nodeDeleteFailedFunc := func(string, string) error {
		return fmt.Errorf("won't remove node")
	}
	podNotFoundFunc := func(action core.Action) (bool, runtime.Object, error) {
	...
	}{{name: "successful attempt to delete node with pods", pods: []string{"p1", "p2"}, drainSuccess: true, nodeDeleteSuccess: true, expectedDeletion: true}, {name: "failed on node delete", pods: []string{"p1", "p2"}, drainSuccess: true, nodeDeleteSuccess: false, expectedDeletion: false}, {name: "successful attempt to delete empty node", pods: []string{}, drainSuccess: true, nodeDeleteSuccess: true, expectedDeletion: true}, {name: "failed attempt to delete empty node", pods: []string{}, drainSuccess: true, nodeDeleteSuccess: false, expectedDeletion: false}}
	for _, scenario := range testScenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if scenario.nodeDeleteSuccess {
				deleteNodeHandler = func(nodeGroup string, node string) error {
					deletedNodes <- node
					return nil
				}
			}
...
func TestScaleDown(t *testing.T) {
	...
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.5, ScaleDownUnneededTime: time.Minute, MaxGracefulTerminationSec: 60, ExpendablePodsPriorityCutoff: 10}
	context := NewScaleTestAutoscalingContext(options, fakeClient, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	scaleDown := NewScaleDown(&context, clusterStateRegistry)
	scaleDown.UpdateUnneededNodes([]*apiv1.Node{n1, n2}, []*apiv1.Node{n1, n2}, []*apiv1.Pod{p1, p2, p3}, time.Now().Add(-5*time.Minute), nil)
...
var defaultScaleDownOptions = config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.5, ScaleDownUnneededTime: time.Minute, MaxGracefulTerminationSec: 60, MaxEmptyBulkDelete: 10, MinCoresTotal: 0, MinMemoryTotal: 0, MaxCoresTotal: config.DefaultMaxClusterCores, MaxMemoryTotal: config.DefaultMaxClusterMemory * units.GiB}
...
func TestScaleDownEmptyMinCoresLimitHit(t *testing.T) {
	...
	config := &scaleTestConfig{nodes: []nodeConfig{{"n1", 2000, 1000, 0, true, "ng1"}, {"n2", 1000, 1000, 0, true, "ng1"}}, options: options, expectedScaleDowns: []string{"n2"}}
	simpleScaleDownEmpty(t, config)
}
...
func TestScaleDownEmptyMinGpuLimitHit(t *testing.T) {
	...
	defer _logClusterCodePath()
	options := defaultScaleDownOptions
	options.GpuTotal = []config.GpuLimits{{GpuType: gpu.DefaultGPUType, Min: 4, Max: 50}, {GpuType: "nvidia-tesla-p100", Min: 5, Max: 50}}
	config := &scaleTestConfig{nodes: []nodeConfig{{"n1", 1000, 1000 * MiB, 1, true, "ng1"}, {"n2", 1000, 1000 * MiB, 1, true, "ng1"}, {"n3", 1000, 1000 * MiB, 1, true, "ng1"}, {"n4", 1000, 1000 * MiB, 1, true, "ng1"}, {"n5", 1000, 1000 * MiB, 1, true, "ng1"}, {"n6", 1000, 1000 * MiB, 1, true, "ng1"}}, options: options, expectedScaleDowns: []string{"n1", "n2"}}
	simpleScaleDownEmpty(t, config)
}
...
func simpleScaleDownEmpty(t *testing.T, config *scaleTestConfig) {
	...
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	scaleDown := NewScaleDown(&context, clusterStateRegistry)
	scaleDown.UpdateUnneededNodes(nodes, nodes, []*apiv1.Pod{}, time.Now().Add(-5*time.Minute), nil)
...
func TestNoScaleDownUnready(t *testing.T) {
	...
	provider.AddNode("ng1", n1)
	provider.AddNode("ng1", n2)
	options := config.AutoscalingOptions{ScaleDownUtilizationThreshold: 0.5, ScaleDownUnneededTime: time.Minute, ScaleDownUnreadyTime: time.Hour, MaxGracefulTerminationSec: 60}
	context := NewScaleTestAutoscalingContext(options, fakeClient, provider)
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	scaleDown := NewScaleDown(&context, clusterStateRegistry)
	scaleDown.UpdateUnneededNodes([]*apiv1.Node{n1, n2}, []*apiv1.Node{n1, n2}, []*apiv1.Pod{p2}, time.Now().Add(-5*time.Minute), nil)
	...
	scaleDown = NewScaleDown(&context, clusterStateRegistry)
	scaleDown.UpdateUnneededNodes([]*apiv1.Node{n1, n2}, []*apiv1.Node{n1, n2}, []*apiv1.Pod{p2}, time.Now().Add(-2*time.Hour), nil)
	scaleDownStatus, err = scaleDown.TryToScaleDown([]*apiv1.Node{n1, n2}, []*apiv1.Pod{p2}, nil, time.Now())
	waitForDeleteToFinish(t, scaleDown)
	assert.NoError(t, err)
	assert.Equal(t, status.ScaleDownNodeDeleteStarted, scaleDownStatus.Result)
...
func TestScaleDownNoMove(t *testing.T) {
	...
	clusterStateRegistry := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	scaleDown := NewScaleDown(&context, clusterStateRegistry)
	scaleDown.UpdateUnneededNodes([]*apiv1.Node{n1, n2}, []*apiv1.Node{n1, n2}, []*apiv1.Pod{p1, p2}, time.Now().Add(5*time.Minute), nil)
...
func TestCalculateCoresAndMemoryTotal(t *testing.T) {
	...
	nodes[6].Spec.Taints = []apiv1.Taint{{Key: deletetaint.ToBeDeletedTaint, Value: fmt.Sprint(time.Now().Unix()), Effect: apiv1.TaintEffectNoSchedule}}
...
func TestFilterOutMasters(t *testing.T) {
	...
	pods := []*apiv1.Pod{BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n2", map[string]string{}), BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "fake-kube-system", "n6", map[string]string{"component": "kube-apiserver"}), BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n3", map[string]string{"component": "kube-apiserver"}), BuildTestPodWithExtra("hidden-name", "kube-system", "n7", map[string]string{"component": "kube-apiserver"}), BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n1", map[string]string{"component": "kube-apiserver-dev"}), BuildTestPodWithExtra("custom-deployment", "custom", "n5", map[string]string{"component": "custom-component", "custom-key": "custom-value"})}
...
func TestCheckScaleDownDeltaWithinLimits(t *testing.T) {
	...
	tests := []testcase{{limits: scaleDownResourcesLimits{"a": 10}, delta: scaleDownResourcesDelta{"a": 10}, exceededResources: []string{}}, {limits: scaleDownResourcesLimits{"a": 10}, delta: scaleDownResourcesDelta{"a": 11}, exceededResources: []string{"a"}}, {limits: scaleDownResourcesLimits{"a": 10}, delta: scaleDownResourcesDelta{"b": 10}, exceededResources: []string{}}, {limits: scaleDownResourcesLimits{"a": scaleDownLimitUnknown}, delta: scaleDownResourcesDelta{"a": 0}, exceededResources: []string{}}, {limits: scaleDownResourcesLimits{"a": scaleDownLimitUnknown}, delta: scaleDownResourcesDelta{"a": 1}, exceededResources: []string{"a"}}, {limits: scaleDownResourcesLimits{"a": 10, "b": 20, "c": 30}, delta: scaleDownResourcesDelta{"a": 11, "b": 20, "c": 31}, exceededResources: []string{"a", "c"}}}
