...
var defaultOptions = config.AutoscalingOptions{EstimatorName: estimator.BinpackingEstimatorName, MaxCoresTotal: config.DefaultMaxClusterCores, MaxMemoryTotal: config.DefaultMaxClusterMemory * units.GiB, MinCoresTotal: 0, MinMemoryTotal: 0}
...
func TestWillConsiderGpuAndStandardPoolForPodWhichDoesNotRequireGpu(t *testing.T) {
	...
	config := &scaleTestConfig{nodes: []nodeConfig{{"gpu-node-1", 2000, 1000 * MiB, 1, true, "gpu-pool"}, {"std-node-1", 2000, 1000 * MiB, 0, true, "std-pool"}}, pods: []podConfig{{"gpu-pod-1", 2000, 1000 * MiB, 1, "gpu-node-1"}, {"std-pod-1", 2000, 1000 * MiB, 0, "std-node-1"}}, extraPods: []podConfig{{"extra-std-pod", 2000, 1000 * MiB, 0, ""}}, expectedScaleUpOptions: []groupSizeChange{{groupName: "std-pool", sizeChange: 1}, {groupName: "gpu-pool", sizeChange: 1}}, scaleUpOptionToChoose: groupSizeChange{groupName: "std-pool", sizeChange: 1}, expectedFinalScaleUp: groupSizeChange{groupName: "std-pool", sizeChange: 1}, options: options}
	simpleScaleUpTest(t, config)
}
...
func TestWillConsiderAllPoolsWhichFitTwoPodsRequiringGpus(t *testing.T) {
	...
	options := defaultOptions
	options.MaxNodesTotal = 100
	config := &scaleTestConfig{nodes: []nodeConfig{{"gpu-1-node-1", 2000, 1000 * MiB, 1, true, "gpu-1-pool"}, {"gpu-2-node-1", 2000, 1000 * MiB, 2, true, "gpu-2-pool"}, {"gpu-4-node-1", 2000, 1000 * MiB, 4, true, "gpu-4-pool"}, {"std-node-1", 2000, 1000 * MiB, 0, true, "std-pool"}}, pods: []podConfig{{"gpu-pod-1", 2000, 1000 * MiB, 1, "gpu-1-node-1"}, {"gpu-pod-2", 2000, 1000 * MiB, 2, "gpu-2-node-1"}, {"gpu-pod-3", 2000, 1000 * MiB, 4, "gpu-4-node-1"}, {"std-pod-1", 2000, 1000 * MiB, 0, "std-node-1"}}, extraPods: []podConfig{{"extra-gpu-pod-1", 1, 1 * MiB, 1, ""}, {"extra-gpu-pod-2", 1, 1 * MiB, 1, ""}, {"extra-gpu-pod-3", 1, 1 * MiB, 1, ""}}, expectedScaleUpOptions: []groupSizeChange{{groupName: "gpu-1-pool", sizeChange: 3}, {groupName: "gpu-2-pool", sizeChange: 2}, {groupName: "gpu-4-pool", sizeChange: 1}}, scaleUpOptionToChoose: groupSizeChange{groupName: "gpu-1-pool", sizeChange: 3}, expectedFinalScaleUp: groupSizeChange{groupName: "gpu-1-pool", sizeChange: 3}, options: options}
	simpleScaleUpTest(t, config)
}
...
func (s assertingStrategy) BestOption(options []expander.Option, nodeInfo map[string]*schedulercache.NodeInfo) *expander.Option {
	...
	if len(s.expectedScaleUpOptions) > 0 {
		...
		assert.Subset(s.t, actualScaleUpOptions, s.expectedScaleUpOptions, "actual %s and expected %s scaleUp options differ", actualScaleUpOptions, s.expectedScaleUpOptions)
		assert.Equal(s.t, len(actualScaleUpOptions), len(s.expectedScaleUpOptions), "actual %s and expected %s scaleUp options differ", actualScaleUpOptions, s.expectedScaleUpOptions)
	}
...
func simpleScaleUpTest(t *testing.T, config *scaleTestConfig) {
	...
	resourceLimiter := cloudprovider.NewResourceLimiter(map[string]int64{cloudprovider.ResourceNameCores: config.options.MinCoresTotal, cloudprovider.ResourceNameMemory: config.options.MinMemoryTotal}, map[string]int64{cloudprovider.ResourceNameCores: config.options.MaxCoresTotal, cloudprovider.ResourceNameMemory: config.options.MaxMemoryTotal})
	provider.SetResourceLimiter(resourceLimiter)
	assert.NotNil(t, provider)
	context := NewScaleTestAutoscalingContext(config.options, fakeClient, provider)
	expander := assertingStrategy{initialNodeConfigs: config.nodes, expectedScaleUpOptions: config.expectedScaleUpOptions, scaleUpOptionToChoose: config.scaleUpOptionToChoose, t: t}
...
func TestScaleUpNodeComingNoScale(t *testing.T) {
	...
	options := config.AutoscalingOptions{EstimatorName: estimator.BinpackingEstimatorName, MaxCoresTotal: config.DefaultMaxClusterCores, MaxMemoryTotal: config.DefaultMaxClusterMemory}
	context := NewScaleTestAutoscalingContext(options, fakeClient, provider)
	nodes := []*apiv1.Node{n1, n2}
	nodeInfos, _ := GetNodeInfosForGroups(nodes, nil, provider, fakeClient, []*extensionsv1.DaemonSet{}, context.PredicateChecker)
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	clusterState.RegisterScaleUp(&clusterstate.ScaleUpRequest{NodeGroup: provider.GetNodeGroup("ng2"), Increase: 1, Time: time.Now(), ExpectedAddTime: time.Now().Add(5 * time.Minute)})
...
func TestScaleUpBalanceGroups(t *testing.T) {
	...
	type ngInfo struct{ min, max, size int }
	testCfg := map[string]ngInfo{"ng1": {min: 1, max: 1, size: 1}, "ng2": {min: 1, max: 2, size: 1}, "ng3": {min: 1, max: 5, size: 1}, "ng4": {min: 1, max: 5, size: 3}}
	...
	options := config.AutoscalingOptions{EstimatorName: estimator.BinpackingEstimatorName, BalanceSimilarNodeGroups: true, MaxCoresTotal: config.DefaultMaxClusterCores, MaxMemoryTotal: config.DefaultMaxClusterMemory}
	context := NewScaleTestAutoscalingContext(options, fakeClient, provider)
	nodeInfos, _ := GetNodeInfosForGroups(nodes, nil, provider, fakeClient, []*extensionsv1.DaemonSet{}, context.PredicateChecker)
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	clusterState.UpdateNodes(nodes, nodeInfos, time.Now())
...
func TestScaleUpAutoprovisionedNodeGroup(t *testing.T) {
	...
	options := config.AutoscalingOptions{EstimatorName: estimator.BinpackingEstimatorName, MaxCoresTotal: 5000 * 64, MaxMemoryTotal: 5000 * 64 * 20, NodeAutoprovisioningEnabled: true, MaxAutoprovisionedNodeGroupCount: 10}
	context := NewScaleTestAutoscalingContext(options, fakeClient, provider)
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	processors := ca_processors.TestProcessors()
...
func TestCheckScaleUpDeltaWithinLimits(t *testing.T) {
	...
	tests := []testcase{{limits: scaleUpResourcesLimits{"a": 10}, delta: scaleUpResourcesDelta{"a": 10}, exceededResources: []string{}}, {limits: scaleUpResourcesLimits{"a": 10}, delta: scaleUpResourcesDelta{"a": 11}, exceededResources: []string{"a"}}, {limits: scaleUpResourcesLimits{"a": 10}, delta: scaleUpResourcesDelta{"b": 10}, exceededResources: []string{}}, {limits: scaleUpResourcesLimits{"a": scaleUpLimitUnknown}, delta: scaleUpResourcesDelta{"a": 0}, exceededResources: []string{}}, {limits: scaleUpResourcesLimits{"a": scaleUpLimitUnknown}, delta: scaleUpResourcesDelta{"a": 1}, exceededResources: []string{"a"}}, {limits: scaleUpResourcesLimits{"a": 10, "b": 20, "c": 30}, delta: scaleUpResourcesDelta{"a": 11, "b": 20, "c": 31}, exceededResources: []string{"a", "c"}}}
