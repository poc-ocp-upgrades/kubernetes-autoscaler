...
func (as *AgentPool) IncreaseSize(delta int) error {
	...
	newDeployment := resources.Deployment{Properties: &resources.DeploymentProperties{Template: &as.template, Parameters: &as.parameters, Mode: resources.Incremental}}
	ctx, cancel := getContextWithCancel()
	defer cancel()
...
func (as *AgentPool) DecreaseTargetSize(delta int) error {
	...
	if curTargetSize+delta < len(nodes) {
		return fmt.Errorf("attempt to delete existing nodes targetSize:%d delta:%d existingNodes: %d", curTargetSize, delta, len(nodes))
	}
