...
func TestBelongs(t *testing.T) {
	...
	assert.True(t, ok)
	invalidNode := &apiv1.Node{Spec: apiv1.NodeSpec{ProviderID: "azure:///subscriptions/test-subscrition-id/resourcegroups/invalid-asg/providers/microsoft.compute/virtualmachinescalesets/agents/virtualmachines/0"}}
	_, err := scaleSet.Belongs(invalidNode)
	assert.Error(t, err)
	validNode := &apiv1.Node{Spec: apiv1.NodeSpec{ProviderID: "azure://" + fakeVirtualMachineScaleSetVMID}}
...
func TestDeleteNodes(t *testing.T) {
	...
	response := autorest.Response{Response: &http.Response{Status: "OK"}}
...
func TestScaleSetNodes(t *testing.T) {
	...
	node := &apiv1.Node{Spec: apiv1.NodeSpec{ProviderID: fakeProviderID}}
