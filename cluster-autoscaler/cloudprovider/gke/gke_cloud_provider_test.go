...
type gkeManagerMock struct{ mock.Mock }
...
func TestGetResourceLimiter(t *testing.T) {
	...
	gkeManagerMock := &gkeManagerMock{}
	resourceLimiter := cloudprovider.NewResourceLimiter(map[string]int64{cloudprovider.ResourceNameCores: 1, cloudprovider.ResourceNameMemory: 10000000}, map[string]int64{cloudprovider.ResourceNameCores: 10, cloudprovider.ResourceNameMemory: 100000000})
	gke := &GkeCloudProvider{gkeManager: gkeManagerMock, resourceLimiterFromFlags: resourceLimiter}
	...
	resourceLimiterGKE := cloudprovider.NewResourceLimiter(map[string]int64{cloudprovider.ResourceNameCores: 2, cloudprovider.ResourceNameMemory: 20000000}, map[string]int64{cloudprovider.ResourceNameCores: 5, cloudprovider.ResourceNameMemory: 200000000})
...
func TestMig(t *testing.T) {
	...
	gkeManagerMock.On("GetMigNodes", mock.AnythingOfType("*gke.GkeMig")).Return([]string{"gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-9j4g", "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-dck1"}, nil).Once()
	...
	mock.AssertExpectationsForObjects(t, gkeManagerMock)
	mig2 := &GkeMig{gceRef: gce.GceRef{Project: "project1", Zone: "us-central1-b", Name: "default-pool"}, gkeManager: gkeManagerMock, minSize: 0, maxSize: 1000, autoprovisioned: true, exist: true, nodePoolName: "default-pool", spec: nil}
...
func TestNewNodeGroupForGpu(t *testing.T) {
	...
	gkeManagerMock.On("GetProjectId").Return("project1").Once()
	gkeManagerMock.On("GetMigTemplateNode", mock.AnythingOfType("*gke.GkeMig")).Return(&apiv1.Node{}, nil).Once()
	systemLabels := map[string]string{gpu.GPULabel: gpu.DefaultGPUType, kubeletapis.LabelZoneFailureDomain: "us-west1-b"}
	extraResources := map[string]resource.Quantity{gpu.ResourceNvidiaGPU: resource.MustParse("1")}
	...
	assert.Equal(t, 0, mig1.MinSize())
	assert.Equal(t, 1000, mig1.MaxSize())
	expectedTaints := []apiv1.Taint{{Key: gpu.ResourceNvidiaGPU, Value: "present", Effect: apiv1.TaintEffectNoSchedule}}
