...
var (
 taintEffectsMap = map[apiv1.TaintEffect]string{apiv1.TaintEffectNoSchedule: "NO_SCHEDULE", apiv1.TaintEffectPreferNoSchedule: "PREFER_NO_SCHEDULE", apiv1.TaintEffectNoExecute: "NO_EXECUTE"}
)
...
func (m *autoscalingGkeClientV1beta1) GetCluster() (Cluster, error) {
 ...
 for _, pool := range clusterResponse.NodePools {
  if pool.Autoscaling != nil && pool.Autoscaling.Enabled {
   nodePools = append(nodePools, NodePool{Name: pool.Name, InstanceGroupUrls: pool.InstanceGroupUrls, Autoscaled: pool.Autoscaling.Enabled, MinNodeCount: pool.Autoscaling.MinNodeCount, MaxNodeCount: pool.Autoscaling.MaxNodeCount, Autoprovisioned: pool.Autoscaling.Autoprovisioned})
  }
 }
...
func (m *autoscalingGkeClientV1beta1) CreateNodePool(mig *GkeMig) error {
 ...
 if gpuRequest, found := spec.ExtraResources[gpu.ResourceNvidiaGPU]; found {
  ...
  gpuConfig := &gke_api_beta.AcceleratorConfig{AcceleratorType: gpuType, AcceleratorCount: gpuRequest.Value()}
 ...
 for _, taint := range spec.Taints {
  ...
  taint := &gke_api_beta.NodeTaint{Effect: effect, Key: taint.Key, Value: taint.Value}
 ...
 config := gke_api_beta.NodeConfig{MachineType: spec.MachineType, OauthScopes: defaultOAuthScopes, Labels: labels, Accelerators: accelerators, Taints: taints}
 autoscaling := gke_api_beta.NodePoolAutoscaling{Enabled: true, MinNodeCount: napMinNodes, MaxNodeCount: napMaxNodes, Autoprovisioned: true}
 createRequest := gke_api_beta.CreateNodePoolRequest{NodePool: &gke_api_beta.NodePool{Name: mig.NodePoolName(), InitialNodeCount: 0, Config: &config, Autoscaling: &autoscaling}}
 registerRequest("node_pools", "create")
 createOp, err := m.gkeBetaService.Projects.Locations.Clusters.NodePools.Create(m.clusterPath, &createRequest).Do()
 if err != nil {
  return err
 }
...
func (m *autoscalingGkeClientV1beta1) waitForGkeOp(op *gke_api_beta.Operation) error {
 ...
 for start := time.Now(); time.Since(start) < m.operationWaitTimeout; time.Sleep(m.operationPollInterval) {
  ...
  if op, err := m.gkeBetaService.Projects.Locations.Operations.Get(fmt.Sprintf(m.operationPath, op.Name)).Do(); err == nil {
