...
func TestGetAcceleratorCount(t *testing.T) {
	...
	testCases := []struct {
		accelerators	[]*gce.AcceleratorConfig
		count		int64
	}{{accelerators: []*gce.AcceleratorConfig{}, count: 0}, {accelerators: []*gce.AcceleratorConfig{{AcceleratorType: "nvidia-tesla-k80", AcceleratorCount: 3}}, count: 3}, {accelerators: []*gce.AcceleratorConfig{{AcceleratorType: "nvidia-tesla-k80", AcceleratorCount: 3}, {AcceleratorType: "nvidia-tesla-p100", AcceleratorCount: 8}}, count: 11}, {accelerators: []*gce.AcceleratorConfig{{AcceleratorType: "other-type", AcceleratorCount: 3}, {AcceleratorType: "nvidia-tesla-p100", AcceleratorCount: 8}}, count: 8}}
	for _, tc := range testCases {
		tb := GceTemplateBuilder{}
...
func TestBuildCapacityMemory(t *testing.T) {
	...
	testCases := []testCase{{physicalMemory: 2 * units.GiB, capacityMemory: 2*units.GiB - 32*units.MiB - kernelReservedMemory, physicalCpu: 1}, {physicalMemory: 4 * units.GiB, capacityMemory: 4*units.GiB - 64*units.MiB - kernelReservedMemory - swiotlbReservedMemory, physicalCpu: 2}, {physicalMemory: 128 * units.GiB, capacityMemory: 128*units.GiB - 2*units.GiB - kernelReservedMemory - swiotlbReservedMemory, physicalCpu: 32}}
...
func TestExtractAutoscalerVarFromKubeEnv(t *testing.T) {
	...
	cases := []struct {
		...
		expect	string
		err	error
	}{{desc: "node_labels", name: "node_labels", env: "AUTOSCALER_ENV_VARS: node_labels=a=b,c=d;node_taints=a=b:c,d=e:f\n", expect: "a=b,c=d"}, {desc: "node_taints", name: "node_taints", env: "AUTOSCALER_ENV_VARS: node_labels=a=b,c=d;node_taints=a=b:c,d=e:f\n", expect: "a=b:c,d=e:f"}, {desc: "malformed node_labels", name: "node_labels", env: "AUTOSCALER_ENV_VARS: node_labels;node_taints=a=b:c,d=e:f\n", err: fmt.Errorf("malformed autoscaler var: node_labels")}}
	for _, c := range cases {
...
func TestParseKubeReserved(t *testing.T) {
	...
	testCases := []testCase{{reserved: "cpu=1000m,memory=300000Mi", expectedCpu: "1000m", expectedMemory: "300000Mi", expectedErr: false}, {reserved: "cpu=1000m,ignored=300Mi,memory=0", expectedCpu: "1000m", expectedMemory: "0", expectedErr: false}, {reserved: "This is a wrong reserved", expectedErr: true}}
	for _, tc := range testCases {
