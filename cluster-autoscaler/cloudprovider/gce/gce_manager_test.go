...
func newTestGceManager(t *testing.T, testServerURL string, regional bool) *gceManagerImpl {
	...
	manager := &gceManagerImpl{cache: GceCache{migs: make([]*MigInformation, 0), GceService: gceService, instancesCache: make(map[GceRef]Mig), machinesCache: map[MachineTypeKey]*gce.MachineType{{"us-central1-b", "n1-standard-1"}: {GuestCpus: 1, MemoryMb: 1}, {"us-central1-c", "n1-standard-1"}: {GuestCpus: 1, MemoryMb: 1}, {"us-central1-f", "n1-standard-1"}: {GuestCpus: 1, MemoryMb: 1}}}, GceService: gceService, projectId: projectId, regional: regional, templates: &GceTemplateBuilder{}, explicitlyConfigured: make(map[GceRef]bool)}
	if regional {
...
func TestDeleteInstances(t *testing.T) {
	...
	instances := []*GceRef{{Project: projectId, Zone: zoneB, Name: "gke-cluster-1-default-pool-f7607aac-f1hm"}, {Project: projectId, Zone: zoneB, Name: "gke-cluster-1-default-pool-f7607aac-c63g"}}
	err := g.DeleteInstances(instances)
	assert.NoError(t, err)
	mock.AssertExpectationsForObjects(t, server)
	instances = []*GceRef{{Project: projectId, Zone: zoneB, Name: "gke-cluster-1-default-pool-f7607aac-f1hm"}, {Project: projectId, Zone: zoneB, Name: "gke-cluster-1-extra-pool-323233232-gdf607aac-9j4g"}}
	err = g.DeleteInstances(instances)
	assert.Error(t, err)
...
func TestGetMigSize(t *testing.T) {
	...
	mig := &gceMig{gceRef: GceRef{Project: projectId, Zone: zoneB, Name: "extra-pool-323233232"}, gceManager: g, minSize: 0, maxSize: 1000}
...
func TestGetMigForInstance(t *testing.T) {
	...
	server.On("handle", "/project1/zones/us-central1-b/instanceGroupManagers/gke-cluster-1-default-pool").Return(getInstanceGroupManager(zoneB)).Once()
	server.On("handle", "/project1/zones/us-central1-b/instanceGroupManagers/gke-cluster-1-default-pool/listManagedInstances").Return(getManagedInstancesResponse1(zoneB)).Once()
	gceRef := &GceRef{Project: projectId, Zone: zoneB, Name: "gke-cluster-1-default-pool-f7607aac-f1hm"}
...
func TestFetchAutoMigsZonal(t *testing.T) {
	...
	min, max := 0, 100
	g.migAutoDiscoverySpecs = []cloudprovider.MIGAutoDiscoveryConfig{{Re: regexp.MustCompile("UNUSED"), MinSize: min, MaxSize: max}}
...
func TestFetchAutoMigsUnregistersMissingMigs(t *testing.T) {
	...
	unregister := &gceMig{gceManager: g, gceRef: GceRef{Project: projectId, Zone: zoneB, Name: gceMigB}, minSize: 1, maxSize: 10}
...
func TestFetchExplicitMigs(t *testing.T) {
	...
	minA, maxA := 0, 100
	minB, maxB := 1, 10
	specs := []string{fmt.Sprintf("%d:%d:https://content.googleapis.com/compute/v1/projects/project1/zones/%s/instanceGroups/%s", minA, maxA, zoneB, gceMigA), fmt.Sprintf("%d:%d:https://content.googleapis.com/compute/v1/projects/project1/zones/%s/instanceGroups/%s", minB, maxB, zoneB, gceMigB)}
	assert.NoError(t, g.fetchExplicitMigs(specs))
	migs := g.GetMigs()
	assert.Equal(t, 2, len(migs))
	validateMig(t, migs[0].Config, zoneB, gceMigA, minA, maxA)
...
func TestGetMigTemplateNode(t *testing.T) {
	...
	regional := false
	g := newTestGceManager(t, server.URL, regional)
	mig := &gceMig{gceRef: GceRef{Project: projectId, Zone: zoneB, Name: "default-pool"}, gceManager: g, minSize: 0, maxSize: 1000}
