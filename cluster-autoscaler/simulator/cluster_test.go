...
func TestUtilization(t *testing.T) {
	...
	mirrorPod4.Annotations = map[string]string{types.ConfigMirrorAnnotationKey: ""}
...
func TestFindPlaceAllOk(t *testing.T) {
	...
	err := findPlaceFor("x", []*apiv1.Pod{new1, new2}, []*apiv1.Node{node1, node2}, nodeInfos, NewTestPredicateChecker(), oldHints, newHints, tracker, time.Now())
...
func TestFindPlaceAllBas(t *testing.T) {
	...
	nodeInfos := map[string]*schedulercache.NodeInfo{"n1": schedulercache.NewNodeInfo(pod1), "n2": schedulercache.NewNodeInfo(), "nbad": schedulercache.NewNodeInfo()}
	...
	newHints := make(map[string]string)
	tracker := NewUsageTracker()
	err := findPlaceFor("nbad", []*apiv1.Pod{new1, new2, new3}, []*apiv1.Node{nodebad, node1, node2}, nodeInfos, NewTestPredicateChecker(), oldHints, newHints, tracker, time.Now())
	assert.Error(t, err)
...
func TestFindNone(t *testing.T) {
	...
	nodeInfos["n1"].SetNode(node1)
	nodeInfos["n2"].SetNode(node2)
	err := findPlaceFor("x", []*apiv1.Pod{}, []*apiv1.Node{node1, node2}, nodeInfos, NewTestPredicateChecker(), make(map[string]string), make(map[string]string), NewUsageTracker(), time.Now())
	assert.NoError(t, err)
}
...
func TestFindEmptyNodes(t *testing.T) {
	...
	pod2.Annotations = map[string]string{types.ConfigMirrorAnnotationKey: ""}
	node1 := BuildTestNode("n1", 1000, 2000000)
	node2 := BuildTestNode("n2", 1000, 2000000)
...
func TestFindNodesToRemove(t *testing.T) {
	...
	emptyNodeToRemove := NodeToBeRemoved{Node: emptyNode, PodsToReschedule: []*apiv1.Pod{}}
	drainableNodeToRemove := NodeToBeRemoved{Node: drainableNode, PodsToReschedule: []*apiv1.Pod{pod1, pod2}}
	pods := []*apiv1.Pod{pod1, pod2, pod3, pod4}
	predicateChecker := NewTestPredicateChecker()
	tracker := NewUsageTracker()
	tests := []findNodesToRemoveTestConfig{{name: "just an empty node, should be removed", candidates: []*apiv1.Node{emptyNode}, allNodes: []*apiv1.Node{emptyNode}, toRemove: []NodeToBeRemoved{emptyNodeToRemove}, unremovable: []*apiv1.Node{}}, {name: "just a drainable node, but nowhere for pods to go to", candidates: []*apiv1.Node{drainableNode}, allNodes: []*apiv1.Node{drainableNode}, toRemove: []NodeToBeRemoved{}, unremovable: []*apiv1.Node{drainableNode}}, {name: "drainable node, and a mostly empty node that can take its pods", candidates: []*apiv1.Node{drainableNode, nonDrainableNode}, allNodes: []*apiv1.Node{drainableNode, nonDrainableNode}, toRemove: []NodeToBeRemoved{drainableNodeToRemove}, unremovable: []*apiv1.Node{nonDrainableNode}}, {name: "drainable node, and a full node that cannot fit anymore pods", candidates: []*apiv1.Node{drainableNode}, allNodes: []*apiv1.Node{drainableNode, fullNode}, toRemove: []NodeToBeRemoved{}, unremovable: []*apiv1.Node{drainableNode}}, {name: "4 nodes, 1 empty, 1 drainable", candidates: []*apiv1.Node{emptyNode, drainableNode}, allNodes: []*apiv1.Node{emptyNode, drainableNode, fullNode, nonDrainableNode}, toRemove: []NodeToBeRemoved{emptyNodeToRemove, drainableNodeToRemove}, unremovable: []*apiv1.Node{}}}
	for _, test := range tests {
		toRemove, unremovable, _, err := FindNodesToRemove(test.candidates, test.allNodes, pods, nil, predicateChecker, len(test.allNodes), true, map[string]string{}, tracker, time.Now(), []*policyv1.PodDisruptionBudget{})
		assert.NoError(t, err)
